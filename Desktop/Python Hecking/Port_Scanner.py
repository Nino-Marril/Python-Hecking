#Multi Threading
import os
from scapy.all import ICMP, IP, srl, TCP, sr
...
def ping(host):
    response = srl(IP(dst=str(host))/ICMP(), timeout=1, verbose=0)
...
def scan_port(args):
    ip, port = args
    response = srl(IP(dst=ip)/TCP(dport=port, flags="S"), timeout=1, verbose=0)
...
def port_scan(ip,ports):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = {executor.submit(ping,host): port for port in ports}
        for i, future in enumerate(as_completed(futures), start=1):
            host =futures[future]
            result = future.result()
            with print_lock:
